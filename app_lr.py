...
st.…
[1:23 pm, 15/07/2026] 🇵🇸✨🇸🇩🤲🇵🇰 somy: import streamlit as st
import pandas as pd
import joblib
from sklearn.model_selection import train_test_split
from sklearn.feature_extraction.text import CountVectorizer
...
try:
    ...
    st.error("❌ email_data.csv not found. Upload it to GitHub")
    st.stop()
...
fig, ax = plt.subplots()
sns.heatmap(cm, annot=True, fmt='d', cmap='Greens', ax=ax) # Changed color to green
